...
export const menuItemTranslations: Record<string, Record<Language, MenuItemTranslation>> = {
  '1': {
    en: {
      name: 'Classic Burger',
    ...
    ru: {
      ...
      description: 'Котлета из говядины ангус, салат, помидор, маринованные огурцы, фирменный соус на булочке бриошь'
    }
  },
  '2': {
    en: {
    ...
    ru: {
      name: 'Салат Цезарь',
      description: 'Хрустящий салат романо, пармезан, гренки, классический соус цезарь'
    }
  }
}
...
export function getTranslatedCategory(category: string, language: Language): string {
  return categoryTranslations[language][category] || category
}

export function getTranslatedMenuItem(itemId: string, language: Language): MenuItemTranslation | undefined {
  return menuItemTranslations[itemId]?.[language] || menuItemTranslations[itemId]?.en
}

export function getUITranslation(key: string, language: Language, params?: Record<string, string | number>): string {
  let translation = uiTranslations[language][key] || uiTranslations.en[key] || key
