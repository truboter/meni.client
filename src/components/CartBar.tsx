...
export function CartBar({ items, onUpdateCart, language, currency, convertPrices }: CartBarProps) {
  ...
  const handleQuantityChange = (index: number, delta: number) => {
    ...
    if (newQuantity <= 0) {
      ...
      const pricePerItem = updatedCart[index].totalPrice / updatedCart[index].quantity
      updatedCart[index] = {
        ...updatedCart[index],
        ...
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
          <SheetContent side="bottom" className="h-[85vh]">
            ...
            <div className="flex flex-col items-center justify-center h-[60vh] text-center">
              <ShoppingCart size={64} weight="thin" className="text-muted-foreground mb-4" />
  ...
  return (
    <>
      <div className="fixed bottom-0 left-0 right-0 z-20 p-4">
        <Button
          onClick={() => setIsOpen(true)}
          className="w-full h-14 text-base shadow-xl flex items-center justify-between px-6"
          size="lg"
          variant="default"
        >
          <div className="flex items-center gap-3">
            <ShoppingCart size={24} weight="bold" />
            <span>{totalItems} {getUITranslation('items', language)}</span>
          </div>
          <span className="font-semibold">{formatPrice(totalPrice, currency, convertPrices)}</span>
        </Button>
      </div>
        ...
        <SheetContent side="bottom" className="h-[85vh]">
                        ...
                        <div className="flex items-center justify-between mt-2">
                          ...
                          <div className="flex items-center gap-3">
                            <span className="font-semibold">
                              {formatPrice(cartItem.totalPrice, currency, convertPrices)}
                            </span>
                            <Button
                              size="icon"
                              variant="ghost"
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-background border-t p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-2xl font-bold">{formatPrice(totalPrice, currency, convertPrices)}</span>
            </div>
